// const FEED_URL = process.env.SUBSTACK_FEED_URL || "https://substack.com/@lexoraafrica/notes";
// const MAX_ITEMS = 5;
...
const FEED_URL =
  ...
  "https://lexoraafrica.substack.com/feed";

const PROFILE_URL =
  process.env.SUBSTACK_PROFILE_URL ||
  "https://substack.com/@lexoraafrica";

const MAX_ITEMS = 7;
const SUMMARY_COUNT = 7;


// Next.js cache refresh every 15 minutes
...
function getItemLink(item, fallbackUrl) {
  const directLink = parseFeedTag(item, "link");
  const guidLink = parseFeedTag(item, "guid");
  const commentsLink = parseFeedTag(item, "comments");

  return directLink || guidLink || commentsLink || fallbackUrl;
}
...
async function getFeedItems(feedUrl) {
  ...
  if (!response.ok) {
    throw new Error(
      `Unable to load Substack feed (${response.status})`
    );
  }
  ...
  const items = [
    ...
    .map((match) => {
      ...
      return {
        ...
        title:
          parseFeedTag(item, "title") ||
          "Untitled post",


        link: getItemLink(item, feedUrl),


        pubDate:
          parseFeedTag(item, "pubDate"),


        description:
          parseFeedTag(item, "description"),
          ...
          {posts.length > 0 ? (
            ...
            >
              ...
              <p className="text-base leading-8">
                ...
                The latest Substack headlines are currently unavailable.
                Please check back shortly or visit the archive.
